...
func ParseLTP(msg []byte) models.LTPInfo {
	...
	ltpInfo := models.LTPInfo{TokenInfo: getToken(msg), SequenceNumber: sequenceNumer,
		ExchangeFeedTimeEpochMillis: exchangeTimestamp, LastTradedPrice: ltp}

	return ltpInfo
}

func ParseQuote(msg []byte) models.Quote {
	quote := models.Quote{}
...
func ParseSnapquote(msg []byte) models.SnapQuote {
	snapquote := models.SnapQuote{}
...
func getBestBuySellData(msg []byte) (bestFiveBuy []models.SmartApiBBSInfo, bestFiveSell []models.SmartApiBBSInfo) {
	bestFiveBuy = make([]models.SmartApiBBSInfo, 0)
	bestFiveSell = make([]models.SmartApiBBSInfo, 0)
	for i := 0; i < 200; i = i + 20 {
		info := models.SmartApiBBSInfo{}
...
func getToken(msg []byte) models.TokenInfo {
	exchangeType := models.ExchangeType(msg[1])
	tokenEnd := 0
	...
	return models.TokenInfo{ExchangeType: exchangeType, Token: string(msg[2 : tokenEnd+1])}
}
