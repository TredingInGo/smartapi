...
import (
	...
	"github.com/TredingInGo/smartapi/parser"
...
type WebSocket struct {
	...
	feedToken           string
	callbacks           callbacks
	subsMap             map[models.SmartStreamSubsMode][]models.TokenInfo
	Conn                *websocket.Conn
	url                 url.URL
	autoReconnect       bool
...
type callbacks struct {
	...
	onSnapquote       func(quote models.SnapQuote)
...
func New(clientID string, feedToken string) *WebSocket {
	ws := &WebSocket{
		...
		subsMap:             make(map[models.SmartStreamSubsMode][]models.TokenInfo),
	}
...
func (ws *WebSocket) SetOnSnapquote(fn func(models.SnapQuote)) {
	if fn != nil {
		ws.callbacks.onSnapquote = fn
	}
}

func (ws *WebSocket) SetOnLTP(fn func(info models.LTPInfo)) {
	if fn != nil {
		ws.callbacks.onLTP = fn
	}
}

func (ws *WebSocket) SetOnQuote(fn func(quote models.Quote)) {
	if fn != nil {
...
func (ws *WebSocket) Subscribe(mode models.SmartStreamSubsMode, tokenIds []models.TokenInfo) error {
	err := ws.subscribeToTokens(mode, tokenIds)
	if err == nil {
		if _, ok := ws.subsMap[mode]; !ok {
			ws.subsMap[mode] = make([]models.TokenInfo, 0)
		}
...
func (ws *WebSocket) subscribeToTokens(mode models.SmartStreamSubsMode, tokenIds []models.TokenInfo) error {
	...
	for {
		...
		default:
			...
			if mType == websocket.BinaryMessage {
				mode := models.SmartStreamSubsMode(msg[0])
...
func (ws *WebSocket) createSubsRequest(mode models.SmartStreamSubsMode, tokenIds []models.TokenInfo) ([]byte, error) {

	exchangeTokenMap := make(map[models.ExchangeType][]string)
	...
	tokenList := make([]models.SubscriptionTokens, 0)
	for k, v := range exchangeTokenMap {
		subscriptionTokens := models.SubscriptionTokens{ExchangeType: k, Tokens: v}
		tokenList = append(tokenList, subscriptionTokens)
	}
	params := models.SubscriptionParam{Mode: mode, TokenList: tokenList}
